...
- HEAD_SHA: 4f281aa6c4ec8f5f4bede698f9f71d0ade844bcc
...
| `identity:` | blocker | persisted record → normalize → instance map | Two persisted records whose embedded `instanceId` collided were both written under that id, so one record — and its quantity and per-instance state — vanished in the one function whose contract is lossless repair. | done — records are keyed by the map key (unique by construction, and what every reference already uses); the embedded id is repaired to match. Asserted in test group 12 |
| `reinterpret:` | flag | instance state → stack key → merge | `stackStateKey` joined entries as `k=v\|k=v`, which is not injective: `{a: 'x\|b=y'}` and `{a: 'x', b: 'y'}` — and `{x: 1}` and `{x: '1'}` — produced the same key, so `mergeStacks` treated distinct states as one stack and discarded the source state. | done — canonical, type-preserving JSON encoding of sorted pairs; asserted in test group 3 |
| `cardinality:` | flag | validate → moveIntoContainer | Validation iterated `state.containers`, so a container instance with **no** capacity map was reported valid while `moveIntoContainer` rejected it for having no positions — two hops disagreeing about the same state. | done — validation is driven from the instances and requires a map per container; normalization creates it. Asserted in test group 12 |
| `cardinality:` | flag | definition → validate → equip | The two-handed invariant was only checked while walking the hand slots, so `twoHanded: true` with `equipSlots: ['head']` passed validation although `equipItem` and `normalizeInventory` both reject it. | done — every equipped two-handed instance must hold both hand references, wherever it is referenced from. Asserted in test group 12 |
