...
- HEAD_SHA: 09aa1a53a9886decc6151431ebc79bc312701d0e
...
Die Browserauswahl ist keine Autorisierung. `projectId`/`worldId` werden serverseitig erneut geprueft. Projektzugriff erfordert GM oder aktive Mitgliedschaft. `supabase/migrations/004_project_membership_security.sql` entfernt die frueheren Self-INSERT/Self-UPDATE-Policies, normalisiert Projekt-Sichtbarkeit auf GM oder aktive Mitgliedschaft und macht Membership-Identity/Role/Status server-/GM-kontrolliert. Self-Service-Beitritt akzeptiert nur den geheimen Projektcode und optional eine eigene Character-ID ueber `join_project_by_code`; die Character-Zuordnung laeuft separat ueber `set_my_project_character`. Inaktive oder gekickte Rows bleiben Denial-Records und koennen nicht per Browser zu `active` umgeschrieben werden. Wenn Projekt und Welt gemeinsam an `character-lore` gesendet werden, muss `worldId === project.world_id` gelten. Direkter World-Kontext ohne Projekt ist nur fuer den World-Creator erlaubt. Ohne Auswahl bleibt die Generierung setting-neutral.
...
| Invalid/missing | Fehlende Auth, kaputte Quota-Infrastruktur, ausgeschoepftes Limit, ungueltige Payloads, fremde Projekt-/Welt-IDs oder manipulierte Membership-Writes muessen vor einem Provider-Fan-out scheitern. | Ohne JWT: 401. Ohne Service-Role bzw. bei nicht erreichbarer/ungueltiger Rate-Limit-RPC: 503 fail-closed. Quota ausgeschoepft: 429. Ungueltige UUID/Payload: 400. Fremde Projekt-/Welt-Lore wird durch GM/aktive Membership/Creator + World-Binding abgewiesen. Browser kann `project_members.status`, `role`, `project_id` oder `user_id` nicht als eigenen Grant schreiben. Browser-Projektlistenfehler laesst den neutralen `Kein Projekt`-Pfad bestehen. | pass |
...
| Membership escalation | Ein User darf durch direkte Tabellenwrites oder durch Delete+Rejoin einer gesperrten Row keine aktive Membership minten. | Canonical V3 RLS entfernt Self-INSERT/Self-UPDATE. Migration 004 entfernt dieselben Legacy/V3 Policy-Namen bei bestehenden Installationen. Join- und Character-Update-RPCs leiten `user_id` aus `auth.uid()` ab und schreiben nur fest erlaubte Felder. Inaktive/gekickte Rows blockieren einen neuen Join, aktive Rows duerfen nur freiwillig verlassen werden. | pass |
...
| `authz-grant:` | resolved | Browser → project_members → lore auth | Der fruehere V3-Policy-Stand erlaubte Self-INSERT/Self-UPDATE auf Membership-Rows und machte die Row als Lore-Grant untrusted. | Canonical V3 RLS + Migration 004 machen Identity/Role/Status/Project server-/GM-kontrolliert; Self-Service nur ueber constrained RPCs. Der Test Gate enthaelt einen statischen Membership-Security-Contract gegen Regression. |
...
## Architecture review
Die persistente DB-Quota ersetzt keinen eigenstaendigen Cache/Queue-Service und nutzt die bereits kanonische Postgres-Trust-Boundary. Das ist fuer diesen kleinen, per-User atomaren Counter weniger Betriebsaufwand als Redis plus eigene Konsistenz-/Failover-Semantik. Die relevante System-Design-Eigenschaft ist Atomicity: konkurrierende Counter-Updates muessen als unteilbare DB-Operation behandelt werden (System Design Reference, S. 111). Die Modulgrenze bleibt eng: Character UI konsumiert das oeffentliche Projects-Modul, die Membership-Services kapseln die constrained RPCs, die Edge Function besitzt AuthZ/Secrets und der Rate-Limit-Helper versteckt den RPC-Transport.

## Verification evidence
- Test Gate Run `32883661351` auf Code-HEAD `09aa1a53a9886decc6151431ebc79bc312701d0e`: PASS
- Typed-strict lint: PASS (33 geaenderte TypeScript-Dateien)
- Typecheck: PASS
- Vite production build: PASS (1797 Module; bestehender Chunk-Size-Warnhinweis bleibt non-blocking)
- Deno check: PASS (6 geaenderte Edge-Function-TypeScript-Dateien)
- Deno tests: 8/8 PASS, davon 4 persistente Rate-Limit-Contract-/Fail-Closed-Tests
- Project membership security contract: PASS; prueft gehärtete RLS-/RPC-Vertraege und verbietet direkte Membership-Grant-Writes im Client-Service
- Secrets diff scan: PASS
- Production dependency audit (`npm audit --omit=dev`): critical=0, high=0, moderate=0, low=0
- Browser E2E fuer denselben Codepfad ist als verpflichtender CI-Job konfiguriert; der nachfolgende Proof-/QA-Head muss vor Merge erneut alle drei Quality-Gate-Jobs gruen haben.
