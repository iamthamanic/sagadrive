...
| authorization | A player may read their adventure's World items but must never write another owner's World definition | Read helper admits owner OR adventure GM OR active member; write helper admits owner only. Insert/update RLS bind World writes to the write helper; Personal owner comes from `getAuthenticatedUserId()`, never client input. Both helpers REVOKE from PUBLIC/anon and GRANT EXECUTE to authenticated | pass (test groups 6, 9) |
