...
- BASE_SHA: 94debc4d60b6597a0b67ff982f6da676440c99c1
- Date: 2026-09-05
- Verdict: CLEAR
...
| `reinterpret:` | blocker | resolveEffectiveWorldProfileId → characters.project_id | Player join only writes `project_members.character_id`, so reading adventure binding from `characters.project_id` alone almost always returned null and the adventure never won precedence. | done — resolve via `characters.project_id` first, then active `project_members` rows for the character |
| `note` | note | core-catalog → consumers | Shallow `Object.freeze` on the Core array left definition objects and nested `equipSlots` mutable, so a consumer could corrupt later loads. | done — deep-freeze |
