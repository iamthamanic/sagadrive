...
- [ ] **Scenarios A–J (issue body)** are not fully automated as browser E2E.
      ...
      - Desktop grid / equipment / containers / quick / mobile segment: static UI
        contracts (#110–#113); Playwright `e2e/character-editor.spec.ts` only
        smokes empty `Inventar 0 / 20` and the 390 Inventar|Ausrüstung segment —
        **not** populated catalog, container, overflow recovery, or World A/B isolation.
- [ ] Focused screenshots under `.qa/evidence/inventory-v2-epic-114/` are listed as
      ...
      for the automated assertions above.
- [x] Misleading “Keine festen Slots” / “statt fester Slots” wording removed from
      README, Core rules (§10 Inventory v2), and active Inventory-v2 UI help
      (`InventorySummaryBar` RuleHelp). Legacy `CharacterInventoryPanel` copy updated
      so it no longer contradicts Inventory v2 rules.
...
| `docs/sagadrive core rules.md` §10.0a | Authoritative 13-point Inventory-v2 contract |
