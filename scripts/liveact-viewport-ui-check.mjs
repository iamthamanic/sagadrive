...
check(/stickyHost/.test(settings), 'gear supports sticky portal host');
check(/avatar-preview-settings-sticky-host/.test(surface), 'surface sticky gear host');
check(/avatar-preview-settings-sticky-host/.test(read('src/app/character/avatar/AvatarPreviewExpandDialog.tsx')), 'expand dialog sticky gear host');
check(/settingsStickyHost/.test(controls), 'controls portal gear to sticky host');
...
check(/liveact-motion-test/.test(settings), 'motion test control');
check(/Motion Test/.test(settings), 'motion test label');
check(/startMotionTest/.test(hook), 'hook starts motion test');
check(/getMotionTestAuditJson/.test(hook), 'hook exports motion test audit');
check(/liveact-motion-test\.ts/.test(read('src/domains/character/liveact/index.ts')) || /liveact-motion-test/.test(read('src/domains/character/liveact/index.ts')), 'domain exports motion test');
check(/LIVEACT_MOTION_TEST_STEPS/.test(read('src/domains/character/liveact/liveact-motion-test.ts')), 'motion test steps defined');
