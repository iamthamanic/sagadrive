...
export function CharacterEditor() {
  ...
  const attributePointsUsed = Object.values(attributes).reduce((sum, value) => sum + value, 0);
              ...
              <Tabs value={activeTab} onValueChange={handleTabChange}>
                ...
                <TabsContent value="background" className="space-y-6">
                  <section className="space-y-4">
                    ...
                    <div className="rounded-lg border border-border bg-muted/15 p-4"><p className="font-medium">Fertigkeitspool · 4 wählen</p><p className="mt-1 text-xs text-muted-foreground">Der Hintergrund definiert vier passende Fertigkeiten. Zwei davon werden trainiert.</p><div className="mt-3 grid gap-3 sm:grid-cols-2">{backgroundSkillPool.map((skill, index) => <SkillSelectField key={index} value={skill} onValueChange={(value) => updateBackgroundPool(index, value)} skillOptions={allSkillKeys} disabledSkillKeys={backgroundSkillPool.filter((current, currentIndex) => currentIndex !== index && isSagaDriveSkillKey(current))} placeholder={`Fertigkeit ${index + 1}`} ariaLabel={`Hintergrund-Fertigkeit ${index + 1}`} />)}</div></div>
                    <div className="rounded-lg border border-border bg-muted/15 p-4"><p className="font-medium">Training · 2 wählen</p><p className="mt-1 text-xs text-muted-foreground">Jede gewählte Fertigkeit erhält 1 der 10 Start-Fertigkeitspunkte.</p><div className="mt-3 grid gap-3 sm:grid-cols-2">{backgroundTraining.map((skill, index) => <SkillSelectField key={index} value={skill} onValueChange={(value) => updateBackgroundTraining(index, value)} skillOptions={selectedBackgroundPool.filter(isSagaDriveSkillKey)} disabledSkillKeys={backgroundTraining.filter((current, currentIndex) => currentIndex !== index && isSagaDriveSkillKey(current))} placeholder={`Training ${index + 1}`} ariaLabel={`Hintergrund-Training ${index + 1}`} disabled={selectedBackgroundPool.length === 0} />)}</div></div>
