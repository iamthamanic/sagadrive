...
class WorldProfileService {
  ...
  async getAccessibleWorldProfilesByIds(ids: string[]): Promise<WorldProfileVm[]> {
    ...
    if (error) throw new Error(`Zugewiesene Welten konnten nicht geladen werden: ${error.message}`);
    return (data ?? []).map((entry) => this.mapToViewModel(entry as WorldProfileDto));
  }

  async getAccessibleWorldProfileById(id: string): Promise<WorldProfileVm> {
    ...
    if (error || !data) throw new Error('Weltprofil nicht gefunden oder nicht zugänglich.');
